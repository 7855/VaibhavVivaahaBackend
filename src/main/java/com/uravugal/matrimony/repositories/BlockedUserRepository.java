...
public interface BlockedUserRepository extends JpaRepository<BlockedUser, Long> {
    // Was declared with a String param against a Long column — MySQL's loose comparison happened
    // to still match, but that's fragile (relies on implicit DB-level coercion, not guaranteed
    // across JPA providers/dialects). blockedByUserId is a Long everywhere else in this file.
    Page<BlockedUser> findByBlockedByUserIdOrderByBlockedAtDesc(Long blockedByUserId, Pageable pageable);
